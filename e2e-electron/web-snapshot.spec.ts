...
import { closeApp, launchApp, openSourceByTitle, type LaunchedApp } from "./harness";
...
test.beforeAll(async () => {
  ...
  handle = await launchApp("web-snapshot");
  page = handle.page;
});
...
test("webpage snapshot in the unified Web viewer: snapshot tab renders → Open Live spawns a live webview tab", async () => {
  // Seed a webpage snapshot via the same API the Library `+` → 网页 → 抓取网页 action
  // uses (the app fetches + snapshots the fixture in-process).
  const seeded = await page.evaluate(async (url) => {
  ...
  // Refresh the Library (header icon — the top-level Refresh button is gone) + open.
