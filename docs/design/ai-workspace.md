...
- **W1 — sessions + one template.** `chatSession` entity + store + `registerChatRoutes` CRUD; extract `<ChatPanel>` + `useChatSession` (the F1 slice); session-list UI. **No AI change.** Delivers exactly the user's "两 chat 一套模板 + session 维护" — the current chat stops being amnesiac.
