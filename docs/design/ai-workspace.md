...
- **W2 — attach + bundle. ✅ SHIPPED (W2-001, 2026-07-05).** `ChatContext.sources[]` (additive, byte-identical at zero); `GET /api/sources/:id/bundle` (bounded excerpt + sealed-filtered notes, directTransport parity); pure `chatContextAssembly` (focused ∪ attachments, sourceId-deduped focused-first, cross-source cap 4000/20/16000); real attachments write path (PATCH + FIFO ops + lazy-create); composer "+" picker + chips; claudeCli weaver dup folded (6 consumers enumerated, pty documented context-blind); CHAT-ONLY. *Deferred to W3:* library multi-select + Ctrl+C/Ctrl+V + drag-drop (§2.5) and synthesis→new-doc (§2.4).
- **W3 — synthesize → new doc. ✅ SHIPPED (W3-001, 2026-07-05).** `chat.synthesize` command + StudyView 生成文档 button → `POST /api/chat/synthesize` (HTTP-only) → structured `{title, markdown}` gen (transcript cap 24000 + W2 attachment bundles, plain chat lane = no profileContext leak, `looksLikeBarePath` guard) → `ingestSource` authored markdown (`#`/`##` = TOC) → opens in a new pane. Plan + impl both adversarially reviewed. *Deferred:* left-sidebar workspace page + default-directory landing; `<ChatPanel>` extraction; clickable bookmark-TOC (V2); regenerate/edit-before-create preview (V1 creates + opens directly).
