...
| ID | Task | Dependencies | Can Parallelize | Primary Files | Implementation Plan | Verification Method | Status |
| --- | --- | --- | --- | --- | --- | --- | --- |
| NOTE-FOCUS-POLISH-001 | Constrain expanded note width and restyle flashcard/quiz full views | NOTE-REF-001, N4D7-001 | No | `src/client/workspace/FocusOverlay.tsx`, `src/client/notes/builtinNoteTypes.tsx`, `src/client/styles.css`, focused note tests | Keep the single `getNoteType().render({mode:"full"})` path, but make CenterView content use a shared reader-width cap. Refresh flashcard and quiz full-mode CSS so interactive cards are compact, left-aligned, readable, and do not turn long answers/options into vertical columns. | Focused note renderer/overlay tests; `npm exec tsc -- --noEmit`; `npm run build`. | Complete |
