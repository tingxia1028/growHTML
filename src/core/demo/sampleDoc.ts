// The self-verify demo document — ONE canonical sample HTML fixture shared by the
// seed script (scripts/seed.ts, node-side ingest) and the onboarding checklist's
// 载入示例文档 button (client-side POST /api/sources/html). Pure data, no imports,
// so both realms can use it. The visible English title stays what the seed script
// always ingested; SAMPLE_SOURCE_TITLE prefixes it with 示例文档 so a seeded sample
// is clearly named in the library AND findable for idempotent re-seeding.

export const SAMPLE_SOURCE_TITLE = "示例文档 · Pressure in Liquids";
